...
class WireOutputProviderTest {
  ...
  @Test
  fun customOutputSettersConfigureNamedProperties() {
    val project = ProjectBuilder.builder().build()
    val output = project.objects.newInstance(CustomOutput::class.java)

    output.setOut("build/generated/custom-wire")
    output.setIncludes(listOf("squareup.dinosaurs.Dinosaur"))
    output.setExcludes(listOf("squareup.geology.Period"))
    output.setExclusive(false)
    output.setOptions(mapOf("a" to "one", "b" to "two"))
    output.setSchemaHandlerFactory(TestSchemaHandlerFactory())
